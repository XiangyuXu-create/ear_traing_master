...
from pages.common import (
    ALL_NOTES, notes_in_range, render_exercise_wav, render_a4_wav,
...
def build_ui(mistake_display):
    ...
    gr.Markdown("""
> 🎵 **训练目标**：听一段随机白键旋律，逐音写出听到的音符序列。
> 先完整播放旋律 → 在下方答案框逐个填入音符 → 提交整条旋律
    """)
    ...
    with gr.Column(visible=True) as l2_exercise:
        ...
        with gr.Row():
            l2_a4_btn = gr.Button("🔔 标准音 A4", scale=1, variant="secondary")
            l2_replay_btn = gr.Button("🔄 重播整段旋律", scale=2)
        with gr.Row(visible=False) as l2_a4_row:
            l2_a4_player = gr.Audio(label="🔔 A4=440Hz", type="filepath", interactive=False, autoplay=True, scale=4)
            l2_a4_close = gr.Button("✕ 收起", scale=1)
        l2_audio = gr.Audio(label="🎧 点击播放", type="filepath", interactive=False)
        l2_note_label = gr.Markdown("")
        l2_radio = gr.Radio(
            choices=notes_in_range("A3", "D5"),
            label="🎯 点击选音（自动填入下方答案框）", interactive=True, value=None,
        )
        l2_answer_box = gr.Textbox(
            label="📝 答案框", interactive=False,
            placeholder="点击上方音名填充...",
        )
        with gr.Row():
            l2_undo_btn = gr.Button("↩ 撤销", scale=1, variant="secondary")
            l2_submit = gr.Button("✅ 提交此旋律", variant="primary", scale=2)
    ...
    def gen(low, high, note_cnt, gap, count, preset):
        ...
        total = len(exercises)
        wavs = [None] * total
        wavs[0] = render_exercise_wav(exercises[0], preset, 0, "l2", gap_ms=int(gap))
        npe = len(answers[0])
    ...
    def submit_melody(st):
        ...
        ans_list = []
        for i, (sel, cor) in enumerate(zip(selections, corrects)):
            ans_list.append({"note_idx": i, "guess": sel, "correct": cor,
                             "is_correct": (sel == cor)})
        ...
        ex += 1
        if ex >= st["total_ex"]:
            st["ex_idx"] = ex  # for finish
            return _finish(st)

        st["ex_idx"] = ex
        if st["wav_paths"][ex] is None:
            st["wav_paths"][ex] = render_exercise_wav(st["exercises"][ex], st["preset"], ex, "l2", gap_ms=st["gap_ms"])
        ...
        try:
            ...
            if 0 <= idx < st["total_ex"]:
                if st["wav_paths"][idx] is None:
                    st["wav_paths"][idx] = render_exercise_wav(st["exercises"][idx], st["preset"], idx, "l2", gap_ms=st["gap_ms"])
                return st["wav_paths"][idx]
        except (ValueError, IndexError):
            pass
        return None

    out = [state, l2_audio, l2_progress, l2_answer_box, l2_radio,
           l2_exercise, l2_result, l2_result_md, l2_result_df, l2_replay_dd, mistake_display]

    l2_gen.click(fn=gen, inputs=[l2_low, l2_high, l2_note_cnt, l2_gap, l2_count, l2_preset], outputs=out)
    l2_radio.change(fn=add_note, inputs=[state, l2_radio], outputs=out)
    l2_undo_btn.click(fn=undo_note, inputs=[state], outputs=out)
    l2_submit.click(fn=submit_melody, inputs=[state], outputs=out)
    ...
    def play_a4(preset):
        path = render_a4_wav(preset)
        return path, gr.update(visible=True), gr.update(interactive=False)

    l2_a4_btn.click(
        fn=play_a4,
        inputs=[l2_preset],
        outputs=[l2_a4_player, l2_a4_row, l2_a4_btn],
    )

    l2_a4_close.click(
        fn=lambda: (gr.update(visible=False), gr.update(interactive=True)),
        outputs=[l2_a4_row, l2_a4_btn],
    )
